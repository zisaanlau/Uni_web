from flask import render_template, request, url_for, redirect, flash,jsonify,Response,make_response
from itsdangerous import TimedJSONWebSignatureSerializer as Serializer
from itsdangerous import BadSignature, SignatureExpired
from core import app, db,executor
from core.models import UserFiles,User,ProcessQueue,BillQueue,AudioQueue,UserVoiceCloneQueue,WxPublicMsg
import logging
import subprocess
import platform
import json
...
def chat_with_file():
    ...
    return json.dumps({'code': 200,
          'status': 'true',
          'msg': '请求成功',"text": answer, "image_path": 'static/image/demo1.jpeg',"video_path": 'static/video/new1.mp4'})
...
[
  ...
  },
  {
    ...
    cmd = cmd +"""   -filter_complex "[0:a][1:a][2:a]concat=n="""+str(len(res))+""":v=0:a=1[out]" -map "[out]" """ + output_path
    print(cmd)
    subprocess.call(cmd, shell=platform.system() != 'Windows')
    # concatenate_mp3_files(mp3_files, output_path)
    audio_url = output_path
    #res.choices[0].message.content  就是我们要的json数据，但是首尾带了一点格式，   ```json\n[....真正的数据.....]```
    #step 1 循环列表生成语音
    #step 2 拼接语音,生成的url赋值audio_url,替换下面的res
    return json.dumps({
              'code': 200,
              'status': 'true',
              'msg': '请求成功',
...
def setAnwser(text,MsgId):
    ...
    headers = {'Content-Type': 'application/json','Authorization': 'Bearer sk-OL2CeOdOK98uhuK1dbeAlA6ZePIWoSbj8iEWip02bwgLGD0A'} 
    response = requests.post(url, headers=headers,json=param, verify=False,stream=True)
    res = response.text
    print(res)
    res = json.loads(res.replace('```json','').replace('```',''))
...
def create_payment_intent():
  # 设置Stripe的密钥
    ...
    intent = stripe.PaymentIntent.create(
      amount=obj['price'], # 金额，单位为最小货币单位（如美元为分）
      currency='usd' # 货币类型
    )
...
@app.route('/api/fulfill_checkout', methods=['POST'])
def fulfill_checkout():
    payload  = request.get_data(as_text = True)
    payloadObj = json.loads(payload)
    # print(payload)
    sig_header = request.headers.get('Stripe-Signature')
    session_id = payloadObj['id']
    userId = payloadObj['data']['object']['client_reference_id']
    print(userId)
    credit = int(payloadObj['data']['object']['amount_total'])
    final_credit = 0
    if credit == 20000:
      final_credit = 2000000
    elif credit == 50000:
      final_credit = 6000000
    item = User.query.filter_by(username = userId).first()
    item.credit += final_credit
    db.session.commit()
    id = int(db.session.query(db.func.max(BillQueue.id)).scalar() or 0)+1
    new_job = BillQueue(
      id=id,
      username=userId,
      create_time = str(datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")),
      price = credit/100,
      add_credit = final_credit,
      )
    db.session.add(new_job)
    db.session.commit()
